...
class RosbridgeWsClient {
    ...
    std::function<void(std::shared_ptr<WsClient>, int, const std::string &)> _on_close;
    std::function<void(std::shared_ptr<WsClient>, const SimpleWeb::error_code &)> _on_error;
    ...
    void start(const std::string &client_name, std::shared_ptr<WsClient> client, const std::string &message) {
        ...
        if (!client->on_message) {
            client->on_message = [client_name](std::shared_ptr<WsClient::Connection> /*connection*/,
        ...
        if (!client->on_close) {
            client->on_close = [client_name](std::shared_ptr<WsClient::Connection> /*connection*/, int status,
        ...
        if (!client->on_error) {
            ...
            client->on_error = [client_name](std::shared_ptr<WsClient::Connection> /*connection*/,
...
public:
    ...
    void advertise(const std::string &client_name, const std::string &topic, const std::string &type,
                   const std::string &id = "") {
        std::unordered_map<std::string, std::shared_ptr<WsClient>>::iterator it = client_map.find(client_name);
        if (it != client_map.end()) {
    ...
    void subscribe(const std::string &client_name, const std::string &topic, const InMessage &callback,
                   const std::string &id = "", const std::string &type = "", int throttle_rate = -1,
                   int queue_length = -1, int fragment_size = -1, const std::string &compression = "") {
        std::unordered_map<std::string, std::shared_ptr<WsClient>>::iterator it = client_map.find(client_name);
        if (it != client_map.end()) {
    ...
    void advertiseService(const std::string &client_name, const std::string &service, const std::string &type,
                          const InMessage &callback) {
        std::unordered_map<std::string, std::shared_ptr<WsClient>>::iterator it = client_map.find(client_name);
        if (it != client_map.end()) {
            std::string message =
                    "{\"op\":\"advertise_service\", \"service\":\"" + service + "\", \"type\":\"" + type + "\"}";
    ...
    void serviceResponse(const std::string &service, const std::string &id, bool result,
                         const rapidjson::Document &values = {}) {
        std::string message = "\"op\":\"service_response\", \"service\":\"" + service + "\", \"result\":" +
                              ((result) ? "true" : "false");
    ...
    void callService(const std::string &service, const InMessage &callback, const rapidjson::Document &args = {},
                     const std::string &id = "", int fragment_size = -1, const std::string &compression = "") {
        ...
        if (callback) {
            ...
            call_service_client->on_message = [](std::shared_ptr<WsClient::Connection> connection,
                                                 std::shared_ptr<WsClient::InMessage> in_message) {
#ifdef DEBUG
...
class RosbridgeWsSubscriber {
public:
    RosbridgeWsSubscriber(std::shared_ptr<RosbridgeWsClient> ws, std::string topic_name, const InMessage &callback) {
        _ws = ws;
        _client_name = "client" + topic_name;
        _ws->addClient(_client_name);
        _ws->subscribe(_client_name, topic_name, callback);
    }

    ~RosbridgeWsSubscriber() {
//        _ws->removeClient(_client_name);
    }
