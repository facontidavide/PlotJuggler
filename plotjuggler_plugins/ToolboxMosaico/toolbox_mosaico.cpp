...
ToolboxMosaico::ToolboxMosaico()
{
  ...
  qRegisterMetaType<std::vector<SequenceInfo>>(
      "std::vector<SequenceInfo>");
  qRegisterMetaType<std::vector<TopicInfo>>(
...
void ToolboxMosaico::init(PJ::PlotDataMapRef& src_data, PJ::TransformsMap& /*transform_map*/)
{
  guardedBoundary("init", [&]() {
    ...
    connect(main_window_, &MainWindow::schemaReady, this,
            &ToolboxMosaico::onSchemaReady);
    connect(main_window_, &MainWindow::queryChanged, this,
            &ToolboxMosaico::onQueryChanged);
  }, [this]() { emit closed(); });
}
                                        ...
                                        const PullResult& result)
{
  ...
  guardedBoundary("onMosaicoDataReady", [&]() {
...
{
  guardedBoundary("onAllFetchesComplete", [&]() {
                                                ...
                                                const PullResult& result)
{
  ...
  const std::string prefix =
      sequence_name.toStdString() + "/" + topic_name.toStdString();
  ...
  for (const auto& batch : result.batches)
  {
    ...
    for (int col = 0; col < batch->num_columns(); ++col)
    {
      ...
      flattenArray(batch->column(col), ts_array, ts_type, ts_is_ns,
                   batch->num_rows(), col_path, created_series);
    }
  }
...
void ToolboxMosaico::onQueryChanged(const QString& query, bool valid)
{
  ...
  guardedBoundary("onQueryChanged", [&]() {
    if (query.isEmpty())
    {
