...
void RangeSlider::drawFloatingLabels(QPainter& painter)
{
  ...
  auto drawLabel = [&](const QRectF& handle_rect, const QString& text) {
    if (text.isEmpty())
    {
      return;
    }
  ...
  {
    QString center_text = mCenterLabelFormatter(static_cast<double>(mLowerValue),
                                                static_cast<double>(mUpperValue));
