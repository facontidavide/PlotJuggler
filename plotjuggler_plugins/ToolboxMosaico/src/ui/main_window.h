...
public:
  ...
  const std::vector<SequenceInfo>& sequences() const { return all_sequences_; }
...
private slots:
  ...
  void onTopicMetadataReady(const QString& sequence_name,
  ...
  void onDeleteRequested(const QStringList& names);
  void onDeleteSucceeded(const QString& sequence_name);
  void onDeleteFailed(const QString& sequence_name, const QString& message);
...
private:
  ...
  void updatePermissions(const QString& level);
  void forwardPermissionsToPanel();
  ...
  // Permission state reported by the server for the current API key.
  // `known=false` means we haven't (or can't) query api_key_status — treat
  // all permissions as optimistically granted; the server is the final gate.
  ...
  // Names with a delete RPC in flight. Prevents double-deleting the same
  // sequence when the user clicks fast before the table refreshes.
  QSet<QString> deleting_;
};
