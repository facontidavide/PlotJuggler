...
public slots:
  ...
  void fetchTopicMetadata(const QString& sequence_name,
  ...
  // pullTopic worker.
  void fetchDataMulti(const QString& sequence_name,
                      const QStringList& topic_names,
                      qint64 start_ns, qint64 end_ns);
  void deleteSequence(const QString& sequence_name);

signals:
  void sequencesReady(const std::vector<SequenceInfo>& sequences);
  void topicsReady(const QStringList& names,
  ...
  void topicMetadataReady(const QString& sequence_name,
                          const QString& topic_name,
                          const TopicInfo& info);
  void dataReady(const QString& sequence_name, const QString& topic_name,
  ...
  void deleteSucceeded(const QString& sequence_name);
  void deleteFailed(const QString& sequence_name, const QString& message);
