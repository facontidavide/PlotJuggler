...
void FetchWorker::fetchSequences()
{
  ...
  {
    emit errorOccurred(QStringLiteral("listSequences failed: ") +
...
void FetchWorker::fetchTopics(const QString& sequence_name)
{
  try
  {
    ...
    if (!result.ok())
    {
      ...
      {
        qDebug() << "[Mosaico fetch] listTopics" << sequence_name
                 << ": NotImplemented after" << ms << "ms (server doesn't support it)";
        emit topicsReady(QStringList{}, std::vector<TopicInfo>{});
      }
      else
      {
        qWarning() << "[Mosaico fetch] listTopics" << sequence_name
                   << ": FAILED after" << ms << "ms —"
                   << QString::fromStdString(result.status().ToString());
        emit errorOccurred(QString::fromStdString(result.status().ToString()));
      }
    ...
    qDebug() << "[Mosaico fetch] listTopics" << sequence_name << ": OK in" << ms
             << "ms," << static_cast<int>(result->size()) << "topics";
    QStringList names;
  ...
  {
    emit errorOccurred(QStringLiteral("listTopics failed: ") +
                       QString::fromUtf8(e.what()));
  }
  catch (...)
  {
...
void FetchWorker::fetchTopicMetadata(const QString& sequence_name,
                                     const QString& topic_name)
{
  try
  {
    ...
    qDebug() << "[Mosaico fetch] getTopicMetadata" << sequence_name << "/"
             << topic_name << ": start";
    const auto t0 = std::chrono::steady_clock::now();
    auto result = client_->getTopicMetadata(sequence_name.toStdString(),
                                            topic_name.toStdString());
    const auto ms = elapsedMs(t0);
    if (!result.ok())
    {
      qDebug() << "[Mosaico fetch] getTopicMetadata" << sequence_name << "/"
               << topic_name << ": FAILED after" << ms << "ms —"
               << QString::fromStdString(result.status().ToString());
      return;
    }
    qDebug() << "[Mosaico fetch] getTopicMetadata" << sequence_name << "/"
             << topic_name << ": OK in" << ms << "ms";
...
void FetchWorker::fetchDataMulti(const QString& sequence_name,
                                 ...
                                 qint64 start_ns, qint64 end_ns)
{
  try
  {
    ...
    TimeRange range;
    if (start_ns > 0) range.start_ns = start_ns;
    if (end_ns > 0) range.end_ns = end_ns;

    std::vector<std::string> topics_std;
    topics_std.reserve(topic_names.size());
    for (const auto& t : topic_names) topics_std.emplace_back(t.toStdString());
    ...
    for (const auto& t : topics_std) progress_state.emplace(t, ProgressState{});

    auto progress = [this, &progress_mu, &progress_state](
                        const std::string& topic, int64_t /*rows*/,
                        int64_t bytes, int64_t total_bytes) {
      bool emit_now = false;
      {
        std::lock_guard<std::mutex> lg(progress_mu);
        auto it = progress_state.find(topic);
        if (it == progress_state.end()) return;
        auto& st = it->second;
    ...
    auto on_done = [this, sequence_name](
      ...
      for (const auto& b : result->batches) {
        if (b) rows += b->num_rows();
      }
      qDebug().noquote() << QString(
                                "[Mosaico fetch] pullTopic %1/%2: OK, %3 batches, %4 rows")
                                .arg(sequence_name)
                                .arg(topic_q)
                                .arg(batches)
                                .arg(rows);
    ...
    qDebug().noquote() << QString(
                              "[Mosaico fetch] pullTopics %1: dispatching %2 topics "
    ...
    const auto t0 = std::chrono::steady_clock::now();
    auto status = client_->pullTopics(sequence_name.toStdString(), topics_std,
                                      range, on_done, progress, &cancel_flag_);
    const auto ms = elapsedMs(t0);
    if (!status.ok())
    {
      ...
      // ourselves here.
      qWarning() << "[Mosaico fetch] pullTopics" << sequence_name
                 << ": dispatch FAILED after" << ms << "ms —"
  ...
  {
    const QString msg = QStringLiteral("pullTopics failed: ") +
                        QString::fromUtf8(e.what());
    for (const auto& topic : topic_names) {
      Q_UNUSED(topic);
      emit errorOccurred(msg);
...
void FetchWorker::deleteSequence(const QString& sequence_name)
{
  try
  {
    if (!client_)
    {
      emit deleteFailed(sequence_name, "not connected");
      return;
    }
    qDebug() << "[Mosaico fetch] deleteSequence" << sequence_name << ": start";
    const auto t0 = std::chrono::steady_clock::now();
    auto status = client_->deleteSequence(sequence_name.toStdString());
    const auto ms = elapsedMs(t0);
    if (status.ok())
    {
      qDebug() << "[Mosaico fetch] deleteSequence" << sequence_name << ": OK in"
               << ms << "ms";
      emit deleteSucceeded(sequence_name);
    }
    else
    {
      qWarning() << "[Mosaico fetch] deleteSequence" << sequence_name
                 << ": FAILED after" << ms << "ms —"
                 << QString::fromStdString(status.ToString());
      emit deleteFailed(sequence_name,
                        QString::fromStdString(status.ToString()));
    }
  }
  catch (const std::exception& e)
  ...
  {
    emit deleteFailed(sequence_name, "unexpected error (unknown)");
  }
}
