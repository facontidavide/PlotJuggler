...
#include "range_slider/RangeSlider.h"
...
void guardedSlot(ElidedLabel* status, const char* where, Fn&& fn)
{
  ...
  catch (...)
  {
    ...
    if (status)
    {
      status->setText(QStringLiteral("Mosaico error (%1): unknown")
                          .arg(QString::fromUtf8(where)));
    }
  }
}
...
void showCopyableWarning(QWidget* parent, const QString& title,
                         const QString& message)
{
  ...
  QObject::connect(copy_btn, &QPushButton::clicked, [message]() {
...
MainWindow::MainWindow(QWidget* parent) : QWidget(parent)
{
  ...
  upload_manager_ = new UploadManager(this);

  connect(sequence_panel_, &SequencePanel::filesDropped,
          this, &MainWindow::onFilesDropped);
  connect(sequence_panel_, &SequencePanel::deleteRequested,
          this, &MainWindow::onDeleteRequested);

  connect(upload_manager_, &UploadManager::uploadStarted,
          sequence_panel_, &SequencePanel::onUploadStarted);
  connect(upload_manager_, &UploadManager::uploadProgress,
          sequence_panel_, &SequencePanel::onUploadProgress);
  connect(upload_manager_, &UploadManager::uploadSucceeded,
          this, [this](const QString& name) {
            sequence_panel_->onUploadSucceeded(name);
            if (client_ && worker_)
              QMetaObject::invokeMethod(worker_, "fetchSequences",
                                        Qt::QueuedConnection);
          });
  connect(upload_manager_, &UploadManager::uploadFailed,
          sequence_panel_, &SequencePanel::onUploadFailed);

  forwardPermissionsToPanel();  // initial state: disconnected, no permissions
...
{
  if (upload_manager_) upload_manager_->shutdown();
  if (worker_thread_)
  {
    worker_thread_->quit();
    worker_thread_->wait();
...
void MainWindow::buildLayout()
{
  ...
  // Certificate/API-key entry — opens the cert dialog where permission state
  // is also shown. Uses the plugin's own SVG rather than the theme's.
  ...
  cert_button_->setToolTip("Certificate & API key (cert optional — system trust store used by default)");
...
void MainWindow::ensureWorkerThread()
{
  ...
  connect(worker_, &FetchWorker::topicMetadataReady, this,
          &MainWindow::onTopicMetadataReady);
  ...
  connect(worker_, &FetchWorker::deleteSucceeded, this,
          &MainWindow::onDeleteSucceeded);
  connect(worker_, &FetchWorker::deleteFailed, this,
          &MainWindow::onDeleteFailed);
...
void MainWindow::onSequencesReady(const std::vector<SequenceInfo>& sequences)
{
  guardedSlot(status_label_, "onSequencesReady", [&]() {
    ...
    // Permission icons stay optimistic: the server is the final gate and
    // rejects any RPC the key isn't authorized for. Surfacing them proactively
    // needs MosaicoClient to expose the connected key's fingerprint so we can
    // call apiKeyStatus() — deferred.
    forwardPermissionsToPanel();

    error_context_ = ErrorContext::None;
    connection_mode_ = attempted_plaintext_fallback_ ? ConnectionMode::Insecure
    ...
    if (!current_query.isEmpty())
    {
      onQueryChanged(current_query,
                     Engine::validate(current_query.toStdString()).valid);
    }
  });
}
...
void MainWindow::onSequenceSelected(const QString& sequence_name)
{
  ...
  // Re-enable panels that a prior delete may have greyed out.
...
void MainWindow::onTopicsReady(const QStringList& names,
                               const std::vector<TopicInfo>& infos)
{
  guardedSlot(status_label_, "onTopicsReady", [&]() {
  cached_topic_infos_ = infos;
  topic_panel_->setLoading(false);
  topic_panel_->populateTopics(names);
  if (names.isEmpty())
  {
...
void MainWindow::onTopicsSelected(const QString& /*sequence_name*/,
                                  const QStringList& topic_names)
{
  ...
  for (const auto& topic : topic_names)
  {
    ...
    if (!worker_) continue;
    if (topic_metadata_in_flight_.contains(cache_key)) continue;
    topic_metadata_in_flight_.insert(cache_key);
    QMetaObject::invokeMethod(worker_, "fetchTopicMetadata",
                              Qt::QueuedConnection,
                              Q_ARG(QString, selected_sequence_),
                              Q_ARG(QString, topic));
  }
...
void MainWindow::onTopicMetadataReady(const QString& sequence_name,
                                      ...
                                      const TopicInfo& info)
{
  guardedSlot(status_label_, "onTopicMetadataReady", [&]() {
    ...
    if (sequence_name != selected_sequence_) return;
    if (!selected_topics_.contains(topic_name)) return;
  ...
  guardedSlot(status_label_, "onDataReady", [&]() {
    ...
    if (pending_fetches_ <= 0)
    {
      ...
      if (cancelling_fetch_)
      {
        ...
        if (worker_) worker_->resetCancel();
...
void MainWindow::onFetchProgress(const QString& topic_name, qint64 bytes,
                                 qint64 total_bytes)
{
  ...
  if (total_bytes > 0)
  {
    ...
    body = QStringLiteral("%1 / %2 (%3%)")
               .arg(formatBytes(bytes), formatBytes(total_bytes))
               .arg(pct);
  }
  else
  {
  ...
  setStatus(QStringLiteral("Fetching %1 (%2/%3): %4")
                .arg(topic_name)
                .arg(idx)
                .arg(total)
                .arg(body));
...
void MainWindow::onFetchClicked()
{
  ...
  QMetaObject::invokeMethod(worker_, "fetchDataMulti", Qt::QueuedConnection,
                            Q_ARG(QString, selected_sequence_),
                            Q_ARG(QStringList, selected_topics_),
                            Q_ARG(qint64, start_ns), Q_ARG(qint64, end_ns));
}
...
void MainWindow::onFetchError(const QString& message)
{
  guardedSlot(status_label_, "onFetchError", [&]() {
    ...
    // available.
    if (in_connect && !attempted_plaintext_fallback_ &&
        cert_path_.isEmpty() && allow_insecure_)
    {
      ...
      auto new_client = std::make_unique<MosaicoClient>(
          uri, kConnectionTimeoutSec, /*pool_size=*/kConnectionPoolSize,
          cert_path_.toStdString(),
    ...
    const bool insecure_blocked = in_connect && !attempted_plaintext_fallback_ &&
                                  cert_path_.isEmpty() && !allow_insecure_;
    if (insecure_blocked)
    {
      display_msg =
    ...
    if (!cancel_drain &&
        (error_context_ == ErrorContext::ExplicitConnect || insecure_blocked))
    {
    ...
    if (error_context_ == ErrorContext::Fetch)
    {
      ...
      if (!cancel_drain)
      {
        showCopyableWarning(this, "Fetch Error", message);
      }
      if (pending_fetches_ <= 0)
      {
        fetch_button_->setText("Download");
...
void MainWindow::restoreState(QSettings& settings)
{
  ...
  const bool has_legacy = settings.contains("tls_cert_path") ||
                          settings.contains("api_key");
  if (has_legacy)
  {
    ...
    if (!migrate_key.isEmpty())
    {
      ...
      server_history_ = promoteToHead(server_history_, migrate_key,
                                      kServerHistoryCap);
      settings.setValue("server_history", server_history_);
      rebuildServerCombo();
      ...
      if (!legacy_cert.isEmpty()) cert_path_ = legacy_cert;
      if (!legacy_key.isEmpty()) api_key_ = legacy_key;
    }
  ...
  if (!current_key.isEmpty())
  {
    ...
    if (!cached_cert.isEmpty()) cert_path_ = cached_cert;
    if (!cached_key.isEmpty()) api_key_ = cached_key;
    settings.endGroup();
    settings.endGroup();
  }
...
// Not currently wired: there is no api_key_status RPC hookup yet. Kept as
// infrastructure so that when the server-reported permission level arrives
// (via a future FetchWorker signal), the call site is one edit away.
void MainWindow::updatePermissions(const QString& level)
{
  permissions_.known = true;
  permissions_.read = permissions_.write = permissions_.del =
      permissions_.manage = false;
  if (level == "manage")
  {
    permissions_.read = permissions_.write = permissions_.del =
        permissions_.manage = true;
  }
  else if (level == "delete")
  {
    permissions_.read = permissions_.write = permissions_.del = true;
  }
  else if (level == "write")
  {
    permissions_.read = permissions_.write = true;
  }
  else if (level == "read")
  {
    permissions_.read = true;
  }
  forwardPermissionsToPanel();
}
...
{
  guardedSlot(status_label_, "onFilesDropped", [&]() {
    // Drop anything that isn't a readable file with a supported extension
    // BEFORE we queue it. Previously a directory or .txt drop would sit in
    // the queue and fail tens of seconds later inside the upload worker
    // with no UI signal.
    ...
    {
      QFileInfo info(path);
      if (info.isFile() && info.isReadable() && acceptsMcapFile(path))
    ...
    if (!rejected.isEmpty())
    {
      showCopyableWarning(
          ...
          QString("Skipped %1 item(s) — only readable .mcap / .bag / .db3 "
    ...
    // Resolve non-colliding names against current sequences + active uploads.
    std::set<QString> existing;
    for (const auto& n : sequence_panel_->existingSequenceNames())
      existing.insert(n);
    for (const auto& n : upload_manager_->activeNames())
      existing.insert(n);

    QList<UploadRequest> reqs;
    for (const auto& path : accepted)
    {
      QString stem = QFileInfo(path).completeBaseName();
      QString name = resolveSequenceName(stem, existing);
      existing.insert(name);
      reqs.append({path, name});
    }

    upload_manager_->setConnection(
        currentServerText(), cert_path_, api_key_);
    upload_manager_->enqueue(reqs);
  });
}

void MainWindow::onDeleteRequested(const QStringList& names)
{
  if (names.isEmpty()) return;

  // Filter out names that already have a delete RPC in flight. The table
  // still shows them because the delete hasn't landed server-side yet, so
  // the user can click Delete again on the same row before we get the
  // refresh back.
  QStringList fresh;
  fresh.reserve(names.size());
  for (const auto& name : names)
  {
    if (!deleting_.contains(name)) fresh.append(name);
  }
  if (fresh.isEmpty()) return;

  const QString title = "Delete sequence" + QString(fresh.size() == 1 ? "" : "s");
  QString body;
  if (fresh.size() == 1)
  {
    body = QString("Permanently delete '%1'?\n\n"
                   "This removes the sequence and all its topics from the "
                   "server. This cannot be undone.")
               .arg(fresh.first());
  }
  else
  {
    body = QString("Permanently delete %1 sequences?\n\n"
                   "This removes all their topics from the server. "
  ...
  {
    deleting_.insert(name);
    QMetaObject::invokeMethod(worker_, "deleteSequence", Qt::QueuedConnection,
                              Q_ARG(QString, name));
  }
}

void MainWindow::onDeleteSucceeded(const QString& sequence_name)
{
  guardedSlot(status_label_, "onDeleteSucceeded", [&]() {
    deleting_.remove(sequence_name);
    // If the deleted sequence was the one currently in view, reset the
    // downstream widgets so the user isn't looking at stale topics/metadata.
    if (sequence_name == selected_sequence_)
    {
      selected_sequence_.clear();
      selected_topics_.clear();
      cached_topic_infos_.clear();
      topic_metadata_cache_.clear();
      topic_metadata_in_flight_.clear();
      topic_panel_->clear();
      topic_panel_->setEnabled(false);
      data_view_panel_->clear();
      data_view_panel_->setEnabled(false);
      range_slider_->setEnabled(false);
      fetch_button_->setEnabled(false);
    }

    // Refresh after each deletion (cheap; last one wins on a burst).
    if (client_ && worker_)
      QMetaObject::invokeMethod(worker_, "fetchSequences", Qt::QueuedConnection);
  });
}

void MainWindow::onDeleteFailed(const QString& sequence_name,
                                 const QString& message)
{
  guardedSlot(status_label_, "onDeleteFailed", [&]() {
    deleting_.remove(sequence_name);
    showCopyableWarning(this, "Delete failed",
                        QString("Could not delete \"%1\":\n%2")
                            .arg(sequence_name, message));
  });
}

void MainWindow::forwardPermissionsToPanel()
{
  // When the permission level is unknown (pre-connect, or the API key isn't
  // a Manage key so api_key_status can't run), optimistically allow the UI —
  // the server is the final gate and will reject unauthorized RPCs.
  const bool can_write = permissions_.known ? permissions_.write : true;
  const bool can_delete = permissions_.known ? permissions_.del : true;

  if (sequence_panel_)
  {
    sequence_panel_->setPermissions(can_write, can_delete);
    sequence_panel_->setConnected(client_ != nullptr);
  }
}
