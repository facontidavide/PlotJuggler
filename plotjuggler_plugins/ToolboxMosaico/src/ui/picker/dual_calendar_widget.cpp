...
void DualCalendarWidget::setExternalRange(const QDate& from, const QDate& to)
{
  ...
  if (from.isValid())
  {
    ...
    const bool to_strictly_later = to.isValid() &&
        (to.year() > new_left_year ||
         (to.year() == new_left_year && to.month() > new_left_month));
  ...
  if (new_left_year == left_year_ && new_left_month == left_month_ &&
      new_right_year == right_year_ && new_right_month == right_month_ &&
      new_from == range_from_ && new_to == range_to_ && !selecting_)
  {
    return;
  }
...
void DualCalendarWidget::rightPrev()
{
  ...
  const bool currently_ge_min =
      right_year_ > right_min_year_ ||
      (right_year_ == right_min_year_ && right_month_ >= right_min_month_);
  if (currently_ge_min &&
